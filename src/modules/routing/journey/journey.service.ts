...
export class JourneyService {
  ...
  public completeTask(beneficiaryId: string, taskId: string): void {
    ...
    if (!journey.completedTaskIds.includes(taskId)) {
      ...
      const nextTask = this.educationPath.find(
        (t) => t.order === (currentTask?.order || 0) + 1,
      );
   ...
   */
  public async routePostWin(
    postWin: PostWin,
    availableBodies: ExecutionBody[],
  ): Promise<string> {
    // 1. Author's Choice (Requirement 2)
    if (postWin.preferredBodyId) {
      const preferred = availableBodies.find(
        (b) => b.id === postWin.preferredBodyId,
      );
