...
class JourneyService {
    ...
    journeys = new Map();
    getOrCreateJourney(beneficiaryId) {
     ...
     */
    validateTaskSequence(journey, taskCode) {
     ...
     */
    completeTask(beneficiaryId, taskId) {
     ...
     */
    async routePostWin(postWin, availableBodies) {
        ...
        if (postWin.preferredBodyId) {
            ...
            if (preferred && this.isBodyCapable(preferred, postWin))
                return preferred.id;
        }
        // 2. Filter by SDG Capability
        const matches = availableBodies.filter(body => postWin.sdgGoals.every(goal => body.capabilities.includes(goal)));
        if (matches.length === 0)
            return core_1.KHALISTAR_ID;
        ...
        return bestMatch ? bestMatch.id : core_1.KHALISTAR_ID;
    }
    isBodyCapable(body, postWin) {
        return postWin.sdgGoals.every(goal => body.capabilities.includes(goal));
    }
    calculateProximity(body, postWin) {
        if (!postWin.location)
            return Infinity;
        // Euclidean distance for coordinate-based routing
        return Math.sqrt(Math.pow(body.location.lat - postWin.location.lat, 2) +
