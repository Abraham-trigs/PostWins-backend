...
exports.PostWinRoutingService = void 0;
const events_1 = require("events");
class PostWinRoutingService extends events_1.EventEmitter {
    taskService;
    journeyService;
    ledgerService;
    MAX_RETRIES = 3;
    BASE_DELAY_MS = 500;
    constructor(taskService, journeyService, ledgerService) {
        super();
     ...
     */
    async emitWithRetry(eventName, payload, attempt = 0) {
        try {
     ...
     */
    async processPostWin(postWin, availableBodies, sdgGoals = ["SDG_4"]) {
        /**
         * STEP 1: Ensure journey exists
         */
        const journey = this.journeyService.getOrCreateJourney(postWin.beneficiaryId);
        /**
         ...
         */
        const verificationRecords = sdgGoals.map((goal) => ({
            sdgGoal: goal,
            requiredVerifiers: 2,
     ...
     */
    async addVerifierApproval(postWin, verifierId, sdgGoal) {
    ...
    async finalizeRouting(postWin, flags) {
        if (!this.shouldEscalate(postWin, flags))
            return;
...
exports.PostWinRoutingService = PostWinRoutingService;
