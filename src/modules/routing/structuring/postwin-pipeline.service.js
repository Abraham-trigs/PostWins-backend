...
exports.PostWinPipelineService = void 0;
class PostWinPipelineService {
    taskService;
    journeyService;
    routingService;
    constructor(taskService, journeyService, routingService) {
     ...
     */
    async intakeAndRoute(message, beneficiaryId, availableBodies, partnerId) {
        // 1. Process intake
        ...
        if (!partialPostWin.taskId)
            partialPostWin.taskId = 'ENROLL'; // default first SDG 4 task
        // 2. Pass through routing & execution logic (includes integrity checks)
        const fullPostWin = await this.routingService.processPostWin(partialPostWin, availableBodies, partialPostWin.sdgGoals);
        return fullPostWin;
    }
    /**
     * Adds a verifier approval to a PostWin
     */
    addVerification(postWin, verifierId, sdgGoal) {
        this.routingService.addVerifierApproval(postWin, verifierId, sdgGoal);
    }
}
exports.PostWinPipelineService = PostWinPipelineService;
/* Example usage:
(async () => {
